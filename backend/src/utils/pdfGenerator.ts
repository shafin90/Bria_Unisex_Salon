...
export const generateBookingTicket = async (booking: any) => {
    let browser;
    try {
        ...
        const totalAmount = booking.service.reduce((sum: number, service: any) => sum + service.servicePrice, 0);
        ...
        <body>
            <div class="ticket">
                ...
                <div class="content">
                    ...
                    <h4>Services:</h4>
                    ${booking.service.map((s: any) => `<div class="info-row"><span>${s.serviceName}</span> <span>$${s.servicePrice}</span></div>`).join('')}
